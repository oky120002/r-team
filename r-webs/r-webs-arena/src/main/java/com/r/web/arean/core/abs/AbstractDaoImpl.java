...
public abstract class AbstractDaoImpl<T> implements AbstractDao<T> {
    ...
    @SafeVarargs
    @Override
    public final List<T> queryByHql(CharSequence hql, KV<String, ?>... keyValues) {
        if (ArrayUtils.isEmpty(keyValues)) {
            return queryByHql(hql, null, -1, -1);
        }
        Map<String, Object> map = new HashMap<String, Object>();
        for (KV<String, ?> kv : keyValues) {
    ...
    @SafeVarargs
    @Override
    public final List<T> queryBySql(CharSequence sql, KV<String, ?>... keyValues) {
        if (ArrayUtils.isEmpty(keyValues)) {
            return queryBySql(sql, null, -1, -1);
        }
        Map<String, Object> map = new HashMap<String, Object>();
