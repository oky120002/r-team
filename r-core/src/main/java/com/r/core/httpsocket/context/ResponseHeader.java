...
public class ResponseHeader implements Serializable {
    ...
    public String bodyToString(String encoding) {
        ...
        if (!ResponseDataType.文本.equals(responseDataType)) {
            throw new ContentTypeErrorException("返回状态 : " + getStatus().toString() + "  Response返回的Body体不是[" + ResponseDataType.文本.name() + "]类型,是[" + responseDataType.name() + "]类型");
        }
        ...
        try {
            ...
            throw new IOReadErrorException("从Response返回的body流转换成文件时流IO转换错误", e1);
        } finally {
    ...
    public BufferedImage bodyToImage() {
        ...
        if (!ResponseDataType.图片.equals(responseDataType) && !ResponseDataType.二进制流.equals(responseDataType)) {
            throw new ContentTypeErrorException(" " + getStatus().toString() + "  Response返回的Body体是[" + responseDataType.name() + "]类型");
        }
    ...
    public File bodyToFile(String fileName) {
        ...
        if (!ResponseDataType.文件.equals(responseDataType) && !ResponseDataType.二进制流.equals(responseDataType)) {
            throw new ContentTypeErrorException("返回状态 : " + getStatus().toString() + "  Response的返回Body是[" + responseDataType.name() + "]");
        }
        InputStream in = bodyToInputStream();
        try {
